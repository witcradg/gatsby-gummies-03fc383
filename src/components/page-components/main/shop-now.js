import React from 'react';
import { CardOverlay } from '../../card-overlay';
// import product from '../../../images/small-banner.webp';

// const alt = 'delta 8 gummies';

export const ShopNow = () => {
	return (
		<div className="text-center mb-3">
			<div className="container-fluid-custom">
				<div className="row">
					<div className="col-sm-6 col-md-4 col-12-440width relative">
						<div className="card bg-3-gummy shop-now-card">
							
							<CardOverlay
								title="DELTA 8 GUMMIES"
								subtitle="12 EDIBLE FLAVORS"
								url="/collections/delta-8-gummies/"
								label="SHOP NOW!"
							/>
						</div>
					</div>
					<div className="col-sm-6 col-md-4 col-12-440width relative">
						<div className="card bg-2-gummy shop-now-card">
							
							<CardOverlay
									title="SALE"
									subtitle="$50 OFF"
									url="/products/buy-2-get-1-free/"
									label="SHOP NOW!"
							/>
						</div>
					</div>
                    {/* <div className="col-sm-6 col-md-4 col-12-440width relative">
						<div className="card bg-1-gummy shop-now-card">
							
							<CardOverlay
								title="FREE 1000MG"
								subtitle="BUY 2 GET 1 FREE"
								url="/collections/free-delta-8/"
								label="SHOP NOW!"
							/>
						</div>
					</div> */}
				</div>
			</div>
		</div>
	);
};
