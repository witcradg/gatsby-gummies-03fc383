...
const IsDelta8THCLegalGeorgiaPage = ({ location, data }) => {
  const md = data.markdownRemark.frontmatter
  const postTitle = `Is Delta 8 THC Legal in ${md.statename} & Can I buy Delta 8 Gummies in ${md.statename}?`
    ...
    <Layout location={location}>
      <IsDelta8THCLegalState title={postTitle} data={data}>
        ...
        <p>
          ...
          {md.statename} passed a piece of state legislation removing all hemp
          derivatives, cannabinoids, terpenes, flavonoids, and all other
          compounds from its comprehensive Drug Abuse Prevention and Control
          Act.
        </p>
        <p>
          This means hemp-derived delta-8 THC is legal under {md.statename}{" "}
          state law, which coincides with federal law.
        </p>
        <h2>
          <strong>
...
export const query = graphql`
  query {
    markdownRemark(frontmatter: { statename: { eq: "Georgia" } }) {
      frontmatter {
        slug
        statename
        distribution
...
export default IsDelta8THCLegalGeorgiaPage
