...
import featuredImage from "../../images/legal-hawaii-delta-8.png"
...
const IsDelta8THCLegalHawaiiPage = ({ location, data }) => {
  const md = data.markdownRemark.frontmatter
  const postTitle = `Is Delta 8 THC Legal in ${md.statename} & Can I buy Delta 8 Gummies in ${md.statename}?`
...
export const query = graphql`
  query {
    markdownRemark(frontmatter: { statename: { eq: "Hawaii" } }) {
      frontmatter {
        slug
        statename
        distribution
...
export default IsDelta8THCLegalHawaiiPage
