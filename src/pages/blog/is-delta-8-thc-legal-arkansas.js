...
const IsDelta8THCLegalArkansasPage = ({ location, data }) => {
  const md = data.markdownRemark.frontmatter
  const postTitle = `Is Delta 8 THC Legal in ${md.statename} & Can I buy Delta 8 Gummies in ${md.statename}?`
  ...
  return (
    <Layout location={location}>
      <IsDelta8THCLegalState title={postTitle} data={data}>
        ...
        <h2>
          <strong>
            What To Look For When Buying Delta-8 Gummies from A Marijuana or
...
export const query = graphql`
  query {
    markdownRemark(frontmatter: { statename: { eq: "Arkansas" } }) {
      frontmatter {
        slug
        statename
        distribution
...
export default IsDelta8THCLegalArkansasPage
