...
const IsDelta8THCLegalRhodeIslandPage = ({ location, data }) => {
  const md = data.markdownRemark.frontmatter
  const postTitle = `Is Delta 8 THC Legal in ${md.statename} & Can I buy Delta 8 Gummies in ${md.statename}?`
    ...
    <Layout location={location}>
      <IsDelta8THCLegalState title={postTitle} data={data} answer={Answer}>
        ...
        <p>
          ...
          delta8gummies.com/locations/rhode-island/ ) and you will be able to
...
export const query = graphql`
  query {
    markdownRemark(frontmatter: { statename: { eq: "Rhode Island" } }) {
      frontmatter {
        slug
        statename
        distribution
...
export default IsDelta8THCLegalRhodeIslandPage
