...
import featuredImage from '../../images/legal-south-carolina-delta-8.png';
...
const IsDelta8THCLegalSouthCarolinaPage = ({ location, data }) => {
    const md = data.markdownRemark.frontmatter;
    const postTitle = `Is Delta 8 THC Legal in ${md.statename} & Can I buy Delta 8 Gummies in ${md.statename}?`;
    ...
    return (
        <Layout location={location}>
            <IsDelta8THCLegalState title={postTitle} data={data}>
                ...
                <p>
                    This means hemp-derived delta-8 THC is legal under{' '}
                    {md.statename} state law, which coincides with federal law.
                </p>
                <h2>
                    <strong>
...
export const query = graphql`
    query {
        markdownRemark(frontmatter: { statename: { eq: "South Carolina" } }) {
            frontmatter {
                slug
                statename
                distribution
...
export default IsDelta8THCLegalSouthCarolinaPage;
