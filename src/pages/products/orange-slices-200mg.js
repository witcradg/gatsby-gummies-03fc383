...
import { OrangeSlicesPouch } from '../../components/products/orange-slices-pouch';
import { prices, formatSeoTitle, formatSeoDescription } from "/src/components/global"

const OrangeSlicesPouchPage = ({ pageContext: { breadcrumb: { crumbs } }, location, data }) => {
	...
	return (
		<Layout location={location}>
			...
			<OrangeSlicesPouch productData={data}/>
		</Layout>
	);
};
...
export const Head = ({location, data}) => {
	...
	// const description = `${pageTitle} $9.99 PLEASE NOTE: We can not ship Delta 8 products to the following states: Alaska, Arizona, Arkansas, Colorado, Delaware`;
...
export const query = graphql`
    query {
        markdownRemark(frontmatter: { cardName: { eq: "OrangeSlicesPouch" } }) {
            frontmatter {
				title
				crumbsPath
				crumbsLabel
...
export default OrangeSlicesPouchPage;
